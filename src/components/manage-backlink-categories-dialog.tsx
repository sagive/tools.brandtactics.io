...
export function ManageBacklinkCategoriesDialog({ onCategoriesChanged }: { onCategoriesChanged?: () => void }) {
  ...
  const handleAddCategory = async () => {
    ...
    const { error } = await supabase.from('backlink_categories').insert([{ name: newName, rank: maxRank + 1 }]);
    if (error) {
  ...
  const handleDeleteCategory = async (id: string) => {
    if (!confirm("Are you sure? This might affect backlinks using this category.")) return;
    const { error } = await supabase.from('backlink_categories').delete().eq('id', id);
    if (error) {
  ...
  const handleUpdateRank = async (id: string, newRank: number) => {
    const { error } = await supabase.from('backlink_categories').update({ rank: newRank }).eq('id', id);
    if (error) {
  ...
  const handleUpdateName = async (id: string, name: string) => {
     const { error } = await supabase.from('backlink_categories').update({ name }).eq('id', id);
     if (error) {
  ...
  return (
    <DialogContent className="sm:max-w-md">
      <DialogHeader>
        <DialogTitle>Manage Backlink Categories</DialogTitle>
      </DialogHeader>
